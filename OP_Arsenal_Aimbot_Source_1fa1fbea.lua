...
-- Outer card: dark navy, draggable, hidden by default
...
MobileBtn.Position = UDim2.new(0.03, 0, 0.82, 0)   -- bottom-left, easy thumb reach
MobileBtn.Size = UDim2.new(0, 64, 0, 64)
MobileBtn.Draggable = true
MobileBtn.Active = true
MobileBtn.Visible = false                            -- shown only when minimised
MobileBtn.ZIndex = 10

MobileBtnCorner.CornerRadius = UDim.new(0, 14)
MobileBtnCorner.Parent = MobileBtn

-- Glassmorphism blur overlay (matches rest of UI)
...
-- Blue top accent stripe (like the title bar on the main panel)
-- Top bar removed
...
-- Big stylised "X" — centred in the middle of the button
MobileBtnX.Name = "MobileBtnX"
MobileBtnX.Parent = MobileBtn
MobileBtnX.BackgroundTransparency = 1
MobileBtnX.BorderSizePixel = 0
MobileBtnX.Position = UDim2.new(0, 0, 0, 4)   -- top of area, leaves room for PROTOCOL below
MobileBtnX.Size = UDim2.new(1, 0, 0, 44)       -- centred horizontally, fills most of height
...
-- "PROTOCOL" pinned to the bottom, fully centred
...
MobileBtnSub.Position = UDim2.new(0, 0, 1, -14)  -- anchored to bottom edge
...
-- Invisible full-size tap target on top (TextButton needed for touch input)
...
-- Roblox Frames ignore touch drag on mobile, so we do it manually via the
-- tap target (MobileBtnClick) which sits on top.
...
InfJump.MouseButton1Down:connect(function()
	...
	if infJumpEnabled then
		...
		connectInfJump()
		-- Re-connect on respawn so it survives death
...
-- Mobile button tap — reopen menu
MobileBtnClick.MouseButton1Down:Connect(function()
	-- Shrink button out then hide
...
uis.InputBegan:Connect(function (input)
	if input.KeyCode == Enum.KeyCode.RightShift then
		if MenuIsToggled == false then
			MobileBtn.Visible = false
			MobileBtn.Size = UDim2.new(0, 64, 0, 64) -- reset size
